...
public class ResendMailUtil {
  ...
  public CreateEmailResponse sendTemplateEmail(
      String toEmail, String subject, String templateId, Map<String, Object> variables) {
    log.info(
        "Sending template email [{}] to {} with variables: {}", templateId, toEmail, variables);

    if (resendApiKey == null || resendApiKey.isBlank()) {
      log.warn(
          "[SIMULATION] RESEND_API_KEY is not configured. Email [{}] to {} simulated with variables: {}",
          templateId,
          toEmail,
          variables);
      return null;
    ...
    try {
      ...
      CreateEmailOptions params =
          CreateEmailOptions.builder()
              ...
              .template(Template.builder().id(templateId).variables(variables).build())
              .build();

      CreateEmailResponse response = resend.emails().send(params);
      log.info(
          "Email [{}] sent successfully via Resend to {}. Resend ID: {}",
          templateId,
          toEmail,
          response.getId());
      return response;
      ...
      log.error(
          "Failed to send template email [{}] via Resend to {}: {}",
          templateId,
          toEmail,
          e.getMessage(),
          e);
