...
public interface ItineraryDayRepository extends JpaRepository<ItineraryDay, UUID> {
  ...
  List<ItineraryDay> findByTripIdAndDayDateBeforeOrTripIdAndDayDateAfter(
      UUID tripIdBefore, LocalDate before, UUID tripIdAfter, LocalDate after);

  List<ItineraryDay> findByTripIdAndDayDateBetween(
      UUID tripId, LocalDate startDate, LocalDate endDate);
  ...
  Optional<ItineraryDay> findByIdAndTripIdForUpdate(
