...
public interface ItineraryItemRepository extends JpaRepository<ItineraryItem, UUID> {
  ...
  @Query(
      "select coalesce(max(i.sortOrder), 0) from ItineraryItem i where i.tripId = :tripId and i.dayId = :dayId")
  int maxSortOrderByTripIdAndDayId(@Param("tripId") UUID tripId, @Param("dayId") UUID dayId);

  @Lock(LockModeType.PESSIMISTIC_WRITE)
  @Query(
      "select i from ItineraryItem i where i.tripId = :tripId and i.dayId = :dayId order by i.sortOrder asc")
  List<ItineraryItem> findByTripIdAndDayIdForUpdate(
      @Param("tripId") UUID tripId, @Param("dayId") UUID dayId);
}
