...
class TripControllerIntegrationTest extends RealInfrastructureTest {
  ...
  void createsTripWithJwtAndPersistsGeneratedDaysThroughFlywayPostgres() throws Exception {
    ...
    String tripId =
        createTrip(USER_A, "Da Nang", startDate.toString(), startDate.plusDays(2).toString());

    mockMvc
        .perform(
            get("/api/trips/{tripId}/itinerary", tripId).header("Authorization", bearer(USER_A)))
  ...
  void hidesAnotherUsersTripAsNotFound() throws Exception {
    String tripId =
        createTrip(USER_A, "Hue", TEST_TODAY.toString(), TEST_TODAY.plusDays(1).toString());

    mockMvc
        .perform(get("/api/trips/{tripId}", tripId).header("Authorization", bearer(USER_B)))
  ...
  void managesItineraryItemLifecycleThroughController() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/trips/{tripId}/itinerary/items/{itemId}", tripId, firstItemId)
    ...
    mockMvc
        .perform(
            put("/api/trips/{tripId}/itinerary/days/{dayId}/items/reorder", tripId, dayId)
                .header("Authorization", bearer(USER_A))
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
    ...
    mockMvc
        .perform(
            delete("/api/trips/{tripId}/itinerary/items/{itemId}", tripId, firstItemId)
                .header("Authorization", bearer(USER_A)))
  ...
  void createsSuggestedItineraryItemWithoutTimes() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/trips/{tripId}/itinerary/days/{dayId}/items", tripId, dayId)
                .header("Authorization", bearer(USER_A))
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
  ...
  void archiveTripRemovesItFromDefaultList() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/trips/{tripId}", tripId).header("Authorization", bearer(USER_B)))
        .andExpect(status().isNoContent());

    mockMvc
        .perform(get("/api/trips").header("Authorization", bearer(USER_B)))
        .andExpect(status().isOk())
  ...
  void listTripsFiltersByDateWindow() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/trips?from=2027-02-01&to=2027-02-28").header("Authorization", bearer(USER_A)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.content[?(@.id == '%s')]".formatted(insideTripId), hasSize(1)))
        .andExpect(
            jsonPath("$.data.content[?(@.id == '%s')]".formatted(outsideTripId), hasSize(0)));
  }
  ...
  private String createTrip(UUID userId, String destination, String startDate, String endDate)
      throws Exception {
    MvcResult result =
        mockMvc
            .perform(
  ...
  private String createItem(String tripId, String dayId, UUID userId, String title)
      throws Exception {
    MvcResult result =
        mockMvc
            .perform(
  ...
  private List<String> dayIds(String tripId, UUID userId) throws Exception {
    MvcResult result =
        mockMvc
            .perform(
                get("/api/trips/{tripId}/itinerary", tripId)
                    .header("Authorization", bearer(userId)))
            .andExpect(status().isOk())
            .andReturn();
  ...
  private int dayVersion(String tripId, String dayId, UUID userId) throws Exception {
    MvcResult result =
        mockMvc
            .perform(
                get("/api/trips/{tripId}/itinerary/days/{dayId}", tripId, dayId)
                    .header("Authorization", bearer(userId)))
            .andExpect(status().isOk())
            .andReturn();
