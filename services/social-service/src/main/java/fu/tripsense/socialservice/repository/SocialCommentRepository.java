...
public interface SocialCommentRepository extends JpaRepository<SocialComment, UUID> {
  ...
  @Query(
      "select c from SocialComment c where c.id = :id and c.postId = :postId and c.deletedAt is null")
  Optional<SocialComment> lockActiveByIdAndPostId(
      @Param("id") UUID id, @Param("postId") UUID postId);
}
