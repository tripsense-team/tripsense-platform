...
public class PlaceRankingServiceImpl implements PlaceRankingService {
  ...
  /**
   * Deterministically ranks a list of PlaceDto items based on: 1. Text relevance match (if query
   * provided) 2. Distance from target location (if lat/lng provided) 3. Rating (0.0 - 5.0) 4. User
   * review count (logarithmic boost)
   */
  @Override
  public List<PlaceDto> rank(
      List<PlaceDto> places, String query, Double targetLat, Double targetLng) {
    ...
    filtered.sort(
        Comparator.comparingDouble(
                (PlaceDto p) -> computeScore(p, normalizedQuery, targetLat, targetLng))
            .reversed());

    return filtered;
  }

  private static final java.util.Set<String> GENERIC_STOP_WORDS =
      java.util.Set.of(
  ...
  public double computeTextMatchScore(PlaceDto place, String query) {
    ...
    String categories =
        (place.getCategories() != null)
            ? String.join(" ", place.getCategories()).toLowerCase()
            : "";
    ...
    // If the query contains distinguishing keywords (e.g. "khong", "ton", "123456", "hawa", "bep",
    // "cuon"),
    ...
    if (!distinguishingWords.isEmpty()) {
      ...
      for (String dw : distinguishingWords) {
        if (isWordContainedIn(nameStripped, dw)
            || isWordContainedIn(addressStripped, dw)
            || isWordContainedIn(categoriesStripped, dw)) {
      ...
      // For short specific queries (1-2 distinguishing words, e.g. "bảo long", "madame lân"),
      // ALL distinguishing words must match to avoid false positives.
      // For longer queries (3+ words), require at least 50% match.
      if (distinguishingWords.size() <= 2) {
        if (matchedDistinguishing < distinguishingWords.size()) {
          return 0.0;
        }
      } else {
        if (matchedDistinguishing == 0 || ratio < 0.50) {
          return 0.0;
        }
      }
    ...
    for (String w : allMeaningfulWords) {
      if (isWordContainedIn(nameStripped, w)
          || isWordContainedIn(addressStripped, w)
          || isWordContainedIn(categoriesStripped, w)) {
        matchedWords++;
      }
    }
  ...
  private boolean isWordContainedIn(String text, String word) {
    ...
    for (String t : tokens) {
      ...
      if ((word.equals("cafe") && t.equals("coffee"))
          || (word.equals("coffee") && t.equals("cafe"))) {
        return true;
      }
    }
    return false;
  ...
  private double computeScore(PlaceDto place, String query, Double targetLat, Double targetLng) {
    ...
    if (targetLat != null && targetLng != null && place.getLocation() != null) {
      double distKm =
          haversineDistanceKm(
              targetLat, targetLng, place.getLocation().getLat(), place.getLocation().getLng());
      if (distKm <= 1.0) {
        score += 15.0;
      } else if (distKm <= 5.0) {
        score += 10.0;
