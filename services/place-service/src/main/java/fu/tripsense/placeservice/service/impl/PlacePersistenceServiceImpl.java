...
public class PlacePersistenceServiceImpl implements PlacePersistenceService {
    ...
    private Place unambiguousDuplicate(PlaceDto incoming) {
        if (!StringUtils.hasText(incoming.getName()) || !StringUtils.hasText(incoming.getAddress())
                || incoming.getLocation() == null) return null;
        List<Place> matches = repository.findByNormalizedName(normalize(incoming.getName())).stream()
                .filter(existing -> existing.getLocation() != null
                        && StringUtils.hasText(existing.getAddress())
                        && normalize(existing.getAddress()).equals(normalize(incoming.getAddress()))
                        && distanceMeters(existing.getLocation().getY(), existing.getLocation().getX(),
                            incoming.getLocation().getLat(), incoming.getLocation().getLng()) <= 100)
                .toList();
    ...
    private static double distanceMeters(double lat1, double lon1, double lat2, double lon2) {
        ...
        double arc = Math.sin(lat / 2) * Math.sin(lat / 2)
    ...
    public PlaceDto toDto(Place place) {
        ...
        PlacePhotoDto primaryPhoto = (place.getPhotos() != null && !place.getPhotos().isEmpty())
                ? new PlacePhotoDto(
        ...
        List<PlacePhotoDto> photoGallery = (place.getPhotos() != null && !place.getPhotos().isEmpty())
