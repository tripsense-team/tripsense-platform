...
@Slf4j
@Service
public class PlaceCacheServiceImpl implements PlaceCacheService {
  ...
  @Override
  public Optional<List<PlaceDto>> getSearchResults(
    ...
    try {
      ...
      if (cached != null) {
        List<PlaceDto> list =
            objectMapper.convertValue(cached, new TypeReference<List<PlaceDto>>() {});
        return Optional.ofNullable(list);
      }
    } catch (Exception ex) {
      log.warn(
          "Redis cache read error for search query '{}': {}", normalizedQuery, ex.getMessage());
    }
    return Optional.empty();
  }

  @Override
  public void putSearchResults(
    ...
    try {
      ...
      log.warn(
          "Redis cache write error for search query '{}': {}", normalizedQuery, ex.getMessage());
    }
  }
  ...
  public Optional<List<AutocompleteSuggestionDto>> getAutocomplete(
    ...
    try {
      String key =
          AUTOCOMPLETE_KEY_PREFIX + hash(cacheKey(normalizedQuery, lat, lng, radius, limit));
      Object cached = redisTemplate.opsForValue().get(key);
      if (cached != null) {
        List<AutocompleteSuggestionDto> list =
            objectMapper.convertValue(
                cached, new TypeReference<List<AutocompleteSuggestionDto>>() {});
        return Optional.ofNullable(list);
      }
    } catch (Exception ex) {
      log.warn(
          "Redis cache read error for autocomplete query '{}': {}",
          normalizedQuery,
          ex.getMessage());
    }
    return Optional.empty();
  ...
  public void putAutocomplete(
    ...
    try {
      ...
      log.warn(
          "Redis cache write error for autocomplete query '{}': {}",
          normalizedQuery,
          ex.getMessage());
    }
  }
  ...
  public void putPlaceDetails(String placeId, PlaceDto place) {
    ...
    try {
      ...
      if (place.getProvider() != null && place.getProviderPlaceId() != null) {
        String providerKey =
            PROVIDER_KEY_PREFIX + place.getProvider() + ":" + place.getProviderPlaceId();
        redisTemplate
            .opsForValue()
            .set(
                providerKey,
                place,
                Duration.ofSeconds(properties.getCache().getProviderTtlSeconds()));
      }
    } catch (Exception ex) {
