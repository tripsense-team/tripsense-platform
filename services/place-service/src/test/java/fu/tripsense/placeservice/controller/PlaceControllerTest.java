...
class PlaceControllerTest {
  ...
  void shouldReturnBadRequestWhenQueryIsBlank() throws Exception {
    mockMvc
        .perform(get("/api/places/search").param("q", ""))
  ...
  void shouldReturnPlacesOnValidSearch() throws Exception {
    ...
    mockMvc
        .perform(get("/api/places/search").param("q", "madame"))
  ...
  void shouldReturnAutocompleteSuggestions() throws Exception {
    AutocompleteSuggestionDto suggestion =
    ...
    mockMvc
        .perform(get("/api/places/autocomplete").param("q", "madame"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
  ...
  void shouldReturn404WhenPlaceNotFound() throws Exception {
    when(placeDetailsService.getDetails("unknown-id", null, null, null))
        .thenReturn(Optional.empty());

    mockMvc
        .perform(get("/api/places/unknown-id"))
