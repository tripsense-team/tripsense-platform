...
public class EmailClientImpl implements EmailClient {
  ...
  public EmailClientImpl(
    ...
    this.restClient = RestClient.builder().baseUrl(emailServiceUrl).build();
  }
  ...
  public void sendTemplateEmail(
    ...
    log.info(
        "Dispatching template email [{}] for {} to EmailService microservice",
        templateName,
        toEmail);
    try {
      restClient
      ...
      log.error(
          "Failed to send template email [{}] via EmailService to {}: {}",
          templateName,
          toEmail,
          e.getMessage());
