...
public class EmailVerificationValidator {
  ...
  public void validateRegistrationEmail(Optional<User> existingUserOpt, String email) {
    if (existingUserOpt.isPresent()) {
      ...
      if (existingUser.getStatus() == UserStatus.ACTIVE) {
        log.warn(
            "Registration validation failed: Email {} is already registered and active", email);
        throw new UserAlreadyExistsException("Email is already registered");
      }
    }
  }

  public EmailVerificationCode validateCodeForVerification(
      String email, String rawCode, LocalDateTime now) {
    EmailVerificationCode codeEntity =
            ...
            .orElseThrow(
                () -> {
                  ...
                  return new InvalidVerificationCodeException(
                      "No pending verification code found for email");
                });
  ...
  public User validateUserForResendCode(Optional<User> userOpt, String email) {
    ...
    if (user.getStatus() != UserStatus.UNVERIFIED) {
      log.warn(
          "Resend code failed: Email {} status is {}, not UNVERIFIED", email, user.getStatus());
