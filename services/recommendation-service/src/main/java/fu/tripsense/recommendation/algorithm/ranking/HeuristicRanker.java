...
public class HeuristicRanker implements Ranker {
    ...
    return candidates.stream()
        ...
        .sorted(
            Comparator.comparingDouble(RankedCandidate::score)
                .reversed()
                .thenComparing(
                    (RankedCandidate value) -> value.breakdown().evidenceCoverage(),
  ...
  private RankedCandidate score(RecommendationContext context, CandidateFeatures features) {
    ...
    applyRequestedDirections(context, values);
    ...
    double retrieval =
        contribution(
            active, values, RankingCriterion.Feature.RETRIEVAL_RELEVANCE, features, denominator);
    double semantic =
        contribution(active, values, RankingCriterion.Feature.SEMANTIC, features, denominator);
    double preference =
        contribution(active, values, RankingCriterion.Feature.PREFERENCE, features, denominator);
    double geographic =
        contribution(active, values, RankingCriterion.Feature.DISTANCE, features, denominator);
    double quality =
        contribution(active, values, RankingCriterion.Feature.RATING, features, denominator);
    double quietness =
        contribution(active, values, RankingCriterion.Feature.QUIETNESS, features, denominator);
    double contextual =
        contribution(active, values, RankingCriterion.Feature.CONTEXT, features, denominator);
    double popularity =
        contribution(active, values, RankingCriterion.Feature.POPULARITY, features, denominator);
    double history =
        contribution(active, values, RankingCriterion.Feature.HISTORY, features, denominator);

    double penalties = 0;
    if (features.preference().available() && features.preference().dislikeConflict())
      penalties -= weights.getDislikeConflictPenalty();
    if (features.history().available() && features.history().previousNegativeFeedback())
      penalties -= weights.getNegativeFeedbackPenalty();
    double finalScore =
        Math.max(
            0,
            Math.min(
  ...
  private Map<RankingCriterion.Feature, Double> activeWeights(RecommendationContext context) {
    ...
    if (!context.rankingCriteria().isEmpty()) {
      for (RankingCriterion criterion : context.rankingCriteria()) {
        active.put(
            criterion.feature(),
            baseWeight(criterion.feature()) * importance(criterion.importance()));
      }
      return active;
    }
    for (RankingCriterion.Feature feature : RankingCriterion.Feature.values())
      active.put(feature, baseWeight(feature));
    return active;
  }
  ...
  private Map<RankingCriterion.Feature, Double> values(CandidateFeatures f) {
    ...
    values.put(
        RankingCriterion.Feature.RETRIEVAL_RELEVANCE, normalizeRrf(f.retrieval().rrfScore()));
    values.put(
        RankingCriterion.Feature.SEMANTIC,
        Math.max(f.semantic().querySimilarity(), f.semantic().userSimilarity()));
    ...
    values.put(
        RankingCriterion.Feature.QUIETNESS,
        f.quietness().score() == null ? 0 : f.quietness().score());
  ...
  private void applyRequestedDirections(
      RecommendationContext context, Map<RankingCriterion.Feature, Double> values) {
    for (RankingCriterion criterion : context.rankingCriteria()) {
      RankingCriterion.Direction naturalDirection = naturalDirection(criterion.feature());
      if (criterion.direction() != naturalDirection && values.containsKey(criterion.feature())) {
        values.computeIfPresent(criterion.feature(), (ignored, value) -> 1.0 - clamp(value));
      }
    }
  }

  private RankingCriterion.Direction naturalDirection(RankingCriterion.Feature feature) {
    // DISTANCE is represented as proximity utility, but its public/raw quantity is minimized.
    return feature == RankingCriterion.Feature.DISTANCE
  ...
  private double contribution(
      Map<RankingCriterion.Feature, Double> active,
      Map<RankingCriterion.Feature, Double> values,
      RankingCriterion.Feature feature,
  ...
  private double normalizeRrf(double value) {
    return value <= 0 ? 0 : value / (value + rrfNormalizationPivot);
  }

  private int upstreamRank(RankedCandidate value) {
    return value.features().sourceEvidence().stream()
        .mapToInt(e -> e.rank())
        .min()
    ...
    if (b.retrieval() > 0)
      values.add(
          new RecommendationReason(
              RecommendationReasonCode.STRONG_RETRIEVAL_MATCH,
              b.retrieval(),
              Integer.toString(f.retrieval().sourceCount())));
    if (b.semantic() > 0)
      values.add(
          new RecommendationReason(
              RecommendationReasonCode.STRONG_SEMANTIC_MATCH,
              b.semantic(),
              format(f.semantic().querySimilarity())));
    if (b.preference() > 0)
      values.add(
          new RecommendationReason(
              RecommendationReasonCode.MATCHES_USER_PREFERENCE,
              b.preference(),
              format(f.preference().categoryMatch())));
    if (b.geographic() > 0)
      values.add(
          new RecommendationReason(
              RecommendationReasonCode.NEAR_TRIP_AREA,
              b.geographic(),
              format(f.geographic().distanceKm())));
    if (b.quality() > 0)
      values.add(
          new RecommendationReason(
              RecommendationReasonCode.HIGH_CONFIDENCE_RATING,
              b.quality(),
              format(f.quality().rawRating())));
    if (b.quietness() > 0)
      values.add(
          new RecommendationReason(
              RecommendationReasonCode.VERIFIED_QUIETNESS,
              b.quietness(),
              format(f.quietness().score())));
    if (b.context() > 0)
      values.add(
          new RecommendationReason(
              RecommendationReasonCode.MATCHES_TRIP_CONTEXT, b.context(), "trip"));
    if (b.popularity() > 0)
      values.add(
          new RecommendationReason(
              RecommendationReasonCode.POPULAR_CHOICE,
              b.popularity(),
              String.valueOf(f.quality().reviewCount())));
    if (f.history().available() && f.history().previouslySaved())
      values.add(new RecommendationReason(RecommendationReasonCode.PREVIOUSLY_SAVED, 0, "saved"));
    return values.stream()
        .sorted(Comparator.comparingDouble(RecommendationReason::contribution).reversed())
        .limit(4)
        .toList();
  }

  private String format(Double value) {
    return value == null ? "unknown" : String.format(Locale.ROOT, "%.4f", value);
  }

  private RecommendationContext defaultContext() {
