...
final class TrustedProxyClientIpResolver {
  ...
  String resolve(ServerWebExchange exchange) {
    ...
    List<InetAddress> forwardedChain =
        parseForwardedChain(exchange.getRequest().getHeaders().get(X_FORWARDED_FOR));
    ...
    static CidrBlock parse(String value) {
      ...
      if (address == null) {
        throw new IllegalArgumentException(
            "Trusted proxy CIDR has an invalid IP address: " + value);
      }
      ...
      try {
        ...
        throw new IllegalArgumentException(
            "Trusted proxy CIDR has an invalid prefix: " + value, exception);
      }
