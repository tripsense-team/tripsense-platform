...
export function useCreatePost() {
  ...
  const create = React.useCallback(
    (payload: CreateSocialPostRequest): Promise<SocialPost> => {
      if (inFlightRef.current) {
        return Promise.reject(
          new Error("Bài viết đang được xử lý, vui lòng chờ."),
        );
      }
      ...
      setSubmitting(true);
      setError(null);
      const operation = socialPostRepository
        .createPost(payload, idempotencyKey)
        .then((created) => {
          idempotencyKeyRef.current = null;
          showFeedback("Đã đăng bài viết.", "success");
          return created;
        })
        .catch((err: unknown) => {
  ...
  const beginNewDraft = React.useCallback(() => {
    if (!inFlightRef.current) idempotencyKeyRef.current = null;
  }, []);
