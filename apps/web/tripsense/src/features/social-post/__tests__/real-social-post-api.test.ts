...
describe("RealSocialPostApi", () => {
  ...
  it("keeps a successful empty response empty", async () => {
    ...
    await expect(
      new RealSocialPostApi().listPosts({ page: 0, size: 10 }),
    ).resolves.toEqual({
      items: [],
      total: 0,
      page: 0,
  ...
  it("sends the idempotency key when creating a post", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValue(
        new Response(
          JSON.stringify({ success: true, data: { id: "post-1" } }),
          { status: 201 },
        ),
      );
    vi.stubGlobal("fetch", fetchMock);

    await new RealSocialPostApi().createPost(
      { content: "one post", media: [] },
      "f0fd5e2f-89e2-49d7-a212-3102c0f4e9f1",
    );

    expect(fetchMock.mock.calls[0][1].headers).toMatchObject({
      "Idempotency-Key": "f0fd5e2f-89e2-49d7-a212-3102c0f4e9f1",
    });
  });

  it("sends the post type before server-side pagination", async () => {
    ...
    await new RealSocialPostApi().listPosts({
      type: "TRIP_SHARE",
      page: 0,
      size: 10,
    });

    expect(fetchMock.mock.calls[0][0]).toContain("type=TRIP_SHARE");
  });
  ...
  it("uses the mock repository only when the explicit flag is true", async () => {
    ...
    expect(realModule.socialPostRepository.constructor.name).toBe(
      "RealSocialPostApi",
    );
    ...
    expect(mockModule.socialPostRepository.constructor.name).toBe(
      "MockSocialPostRepository",
    );
  });
});
