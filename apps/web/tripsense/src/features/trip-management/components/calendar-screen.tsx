...
export function CalendarScreen({
  ...
  return (
    <section className="min-h-screen px-6 py-10 sm:px-8 lg:px-12 xl:px-16">
      ...
      {loading ? (
        <LoadingState className="mt-12" text="Loading calendar..." />
      ) : null}
        ...
        <div className="min-w-[980px]">
          <div className="grid grid-cols-[92px_repeat(7,minmax(120px,1fr))] border-b border-border">
            ...
            {days.map((day) => (
              <div
                key={day.toISOString()}
                className="pb-4 text-center text-base font-medium text-muted-foreground"
              >
          ...
          <div className="grid grid-cols-[92px_repeat(7,minmax(120px,1fr))]">
            <div className="border-r border-border py-4 text-right text-sm text-muted-foreground">
              all-day
            </div>
            {days.map((day) => (
              <CalendarCell
            ...
            {hours.map((hour) => (
              <React.Fragment key={hour}>
                <div className="border-r border-t border-border px-3 py-6 text-right text-sm font-medium text-muted-foreground">
                  {hour}
                </div>
                {days.map((day) => (
                  <CalendarCell
...
function CalendarCell({
  ...
  const matchingTrip = allDay
    ? trips.find((trip) => isDateInsideTrip(day, trip))
    : null;
  return (
    <div className="min-h-24 border-r border-t border-border p-2">
      {matchingTrip && (
        <Link
          ...
          className="block rounded-xl bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground"
        >
