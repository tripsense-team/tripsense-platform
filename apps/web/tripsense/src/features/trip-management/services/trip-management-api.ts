...
export function listTrips(
...
): Promise<PageResponse<TripResponse>> {
  ...
  return unwrap(
    apiClient<ApiResponse<PageResponse<TripResponse>>>(
      `/api/trips?${searchParams.toString()}`,
    ),
  );
}
...
export function createTrip(payload: CreateTripRequest): Promise<TripResponse> {
  return unwrap(
    apiClient<ApiResponse<TripResponse>>("/api/trips", {
      method: "POST",
...
export function updateTrip(
  ...
  return unwrap(
    apiClient<ApiResponse<TripResponse>>(`/api/trips/${tripId}`, {
      method: "PATCH",
      body: JSON.stringify(payload),
    }),
  );
...
export async function deleteTrip(tripId: string): Promise<void> {
  await apiClient<ApiResponse<void>>(`/api/trips/${tripId}`, {
    method: "DELETE",
  });
}
...
export function getItinerary(tripId: string): Promise<ItineraryResponse> {
  return unwrap(
    apiClient<ApiResponse<ItineraryResponse>>(`/api/trips/${tripId}/itinerary`),
  );
}
...
export function getItineraryDay(
  ...
  return unwrap(
    apiClient<ApiResponse<ItineraryDayResponse>>(
...
export function createItineraryItem(
  ...
  return unwrap(
    apiClient<ApiResponse<ItineraryItemResponse>>(
